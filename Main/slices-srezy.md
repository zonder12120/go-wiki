...
То есть сам срез в себе  не хранит его содержимое, а его размер зависит от разрядности системы. Если ОС имеет разрядность 64 бита, то **array, len** и **cap** занимают в памяти по 8 байт, а слайс в сумме весть **24 байта**, если разрядность 32 бита, то, очевидно, слайс будет весить **12 байт.**
...
Хочу напомнить, что сравнивать слайсы между собой в Go напрямую нельзя, их можно сравнивать только с **nil**
...
А, чуть не забыл про фигню, которой никто обычно не пользуется, но она меня в покое не оставила.
...
Мы так же ссылаемся на тот же самый базовый массив, просто для слайса нам ограничили количество ячеек памяти. В целом функционал понятен, но не могу придумать кейсы для использования.