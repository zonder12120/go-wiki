...
**Регистры общего назначения** обычно используются для хранения промежуточных результатов вычислений и других временных данных, которые понадобятся в процессе выполнения программы. Эти регистры могут быть доступны для программиста для хранения промежуточных значений, передачи параметров и других операций.
...
4. **FLAGS:**
   ...
   -  Хранит флаги состояния процессора, такие как флаги нуля, переноса, знака и переполнения. Эти флаги используются для принятия решений в программе, основанных на результатах предыдущих операций.
...
```Assembly
; Использование регистра EAX для арифметических операций
    mov eax, 5          ; Кладем 5 в регистр EAX
    add eax, 10         ; Добавляем 10 к регистру EAX (итог: 15)

; Использование регистра EBX для хранения данных и арифметических операций
    mov ebx, eax        ; Копируем значение EAX в EBX (EBX теперь 15)
    sub ebx, 3          ; Вычитаем 3 из EBX (итог: 12)

; Использование регистра ECX в качестве счетчика
    mov ecx, 3          ; Кладем 3 в регистр ECX
loop_start:
    dec ecx             ; Уменьшаем ECX на 1
    jz loop_end         ; Если ECX = 0, переходим на метку loop_end
    add eax, ecx        ; Добавляем ECX к EAX (2 + 15, затем 1 + 17, в итоге EAX хранит 18)
    jmp loop_start      ; Переходим на метку loop_start
loop_end:

; Использование регистра EDX для умножения и деления
    mov edx, 2          ; Кладем 2 в регистр EDX
    imul eax, edx       ; Умножаем EAX на EDX (итог: 36)
```
