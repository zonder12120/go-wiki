...
Процессы могут работать в 2-х режимах:

-  **Пользовательский режим (User Mode)**: В этом режиме выполняются обычные приложения. В пользовательском режиме доступ к аппаратным ресурсам ограничен, чтобы предотвратить случайные или злонамеренные действия, которые могли бы нарушить работу системы.
...
Эта api называется **системными вызовами.**
...
-  `exec()`: Замещает текущий процесс новым процессом (запускает другую программу). Это семейство вызовов (`execl()`, `execp()`, и т.д.) позволяет выполнить новый исполняемый файл.
