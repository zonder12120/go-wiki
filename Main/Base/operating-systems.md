...
Давай так, функций операционок предостаточно, но их все перечислять сейчас особого смысла нет, по крайней мере для старта в Гошке. Иначе копать вглубь можно бесконечно долго, операционные системы - это целая наука. На этом я буду основываться на протяжении всей статьи. Слишком глубоко копать не буду.
...
:::info:true Что значит d в имени systemD

Название systemd соответствует **соглашению** Unix об именовании демонов путем добавления буквы d.

**Демон** в Unix-подобных ОС -- это фоновый процесс, который работает в системе без прямого взаимодействия с пользователем.
...
Но ведь у нас постоянно запущено в ОС огромное количество процессов, как тогда они все исоплняются на компьютере, да ещё и одновременно? Тут влетает с двух ног планировщик. Но про него ниже.

#### **Состояния потоков**:

-  **Blocked/Waiting**: Поток приостановлен, так как ожидает внешнего события (завершения I/O, освобождения мьютекса, сигнала от другого потока и т.д.). В это время он **не потребляет** ресурсы CPU.

-  **Runnable**: Поток готов к выполнению и ждет, когда планировщик выделит ему время на ядре CPU. Если Runnable-потоков больше, чем доступных ядер, возникает конкуренция, и задержки планирования влияют на производительность.

-  **Running**: Поток активен и выполняет инструкции на ядре CPU.

#### **Типы выполняемой работы**:

-  **CPU-Bound**: Работа, где основное время тратится на вычисления (арифметические операции, обработку данных в памяти). Такие задачи редко блокируются на I/O, но могут иметь краткие периоды ожидания (например, из-за кэш-промахов).

-  **I/O-Bound**. Работа, где потоки часто блокируются, ожидая завершения операций ввода-вывода (чтение/запись на диск, сетевые запросы, системные вызовы). В это время они освобождают CPU для других задач.
...
Планировщик ОС обеспечивает иллюзию параллельного выполнения множества потоков, за счёт быстрого переключения между потоками. Это называется **конкурентностью.**

### **Конкурентность**

Слово само за себя говорящее. Потоки **конкурируют** за процессорное время, постоянно перетягивая канат каждый в свою сторону, заставляя переключать контекст.

Например, потока ОС у нас два, а процессор одноядерный однопоточный. Эти 2 потока будут выполняться как будто одновременно, но не параллельно, а постоянно переключаясь с одного на другой путём переключения контекста.

#### Сравнение **параллелизма** и конкурентности

![](./operating-systems-4.png)

На картинке представлено 2 процессора: первый однопоточный, второй имеет два потока.

Напоминаю, что потоки процессора - это не потоки ОС, но только один поток ОС может работать на одном потоке процессора единовременно, то есть соотношение 1 к 1. Правда, потоки ОС могут сменять друг друга в 1 потоке процессора (решение принимает планировщик ОС).

Оба процессора выполняют одни и те же задачи, помеченные цветами (задача 1 - задача 4).

**Первый процессор** распределяет своё **процессорное время** между всемя четыремя задачами, чтобы выполнять их для пользователя **как будто** одновременно. Но по факу процессорное время распределяется между этими четыремя задачами (потоками ОС) в зависимости от приоритета задачи, стараясь равномерно и справедливо уделять время каждой задаче.

**Второй процессор** (с двумя потоками) делает всё тоже самое, но теперь он может выполнять не 4 задачи в одном потоке, а пустить в один поток 2 задачи, а во второй поток другие 2 задачи, что позволяет выделять каждой задаче **больше** процессорного времени.

Вторым процессором задачи выполняются не только **конкурентно**, но и **параллельно**, выполняя больше работы за единицу времени (в нашем случае 1 сек), чем однопоточный процессор. Это называется **конкурентный параллелизм**.
...
Довольно прикольное название, такая метафора на потоки и железную дорогу.
