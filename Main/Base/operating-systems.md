...
Напоминаю, что потоки процессора - это не потоки ОС, но только один поток ОС может работать на одном потоке процессора, то есть соотношение 1 к 1. Правда, один поток процессора может переключаться между потоками ОС (решение принимает планировщик ОС).
