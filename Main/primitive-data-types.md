...
Типы данных в Go делятся на две категории: **базовые** и **составные**. К первым относятся целые, вещественные и комплексные числа, строки, байты и руны. К составным типам данных относятся массивы, структуры, срезы, мапы, каналы, указатели, функции и интерфейсы. Здесь мы разберёмся с базовыми, так как составные нужно разбирать каждый по отдельности
...
Например русские буквы занимают 2 байта, а иероглифы вообще 4. Получается, для строки из русских букв “хуй” в массиве байт будет 6 байт: по 2 подряд на каждую букву. То есть первые 2 для буквы “х” и так далее.
...
nil - это обзначение отсуствия какого-либо значения. Точка. Это однозначное применение nil в Go. Вот как программисту уже не сильно важно, как работает оно под капотом, но интересно, конечно, поэтому расскажу.
...
-  Это значение **не обязательно** совпадает с **фактическим нулевым адресом** в памяти, хотя на практике оно часто совпадает.
